...
############################
def get_page_contents(pg_name, pg_size, offset, drafts=False):
    db.select('content', where="page=$pg and draft=d", vars = dict(pg=pg_name, d=int(drafts)), offset=offset, limit = pg_size, order="created DESC")
